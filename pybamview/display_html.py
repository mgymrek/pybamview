...
def GetHeader(bamfiles, region, minpos, maxpos, reffile, samples):
    ...
    header_html += "<script language=javascript type='text/javascript'>var chrom=\"%s\";\n%s</script>"\
        %(chrom,open("%s/pybamview.js"%JS_PREFIX,"r").read())
...
def GetReference(reference_string, chrom, pos, positions):
    ...
    for i in range(len(reference_string)):
        ...
        reference_html += "<td class='%s_%s' id='ref%s' style='background-color:%s;'><font class='ref'><b>%s</b></font></td>"%(chrom, positions[i], i, color, reference_string[i])
    reference_html += "</tr>"
...
def GetAlignment(alignments_by_sample, numcols, chrom, pos, positions):
    ...
    for sample in alignments_by_sample:
        ...
        aln_html += "<div class='sample' onclick='toggleDiv(\"%s\");'>%s (show/hide)</div><div>how did you find me?</div>"%(sample, sample)
        aln_html += "<div id='%s'>"%sample
        aln_html += "<table>"
